...
function CentersPage() {
  ...
  const [centers, setCenters] = useState<{ email: string; created_at: string }[]>([]);
  const [students, setStudents] = useState<Student[]>([]);
  const [open, setOpen] = useState(false);

  const load = async () => {
    const [c, s] = await Promise.all([
      supabase.from("allowed_managers" as any).select("email, created_at").eq("role", "center").order("created_at", { ascending: false }),
        ...
        <div className="bg-[#fbf6e7] border-4 border-foreground shadow-[6px_6px_0px_0px_#1a1410] overflow-x-auto">
          <table className="w-full text-left font-serif-news text-sm">
            <thead className="bg-[#f4ecd8] border-b border-foreground/30">
              <tr>
                <th className="p-4 font-sans font-bold text-xs uppercase tracking-wider text-[#6b3e1a]">Center Email</th>
            ...
            <tbody>
              {centers.map((c) => {
                ...
                return (
                  <tr key={c.email} className="border-t border-foreground/10 hover:bg-[#f4ecd8]/40">
                    <td className="p-4 font-bold">{c.email}</td>
              ...
              {centers.length === 0 && (
                <tr><td colSpan={4} className="p-10 text-center italic text-[#6b3e1a]">No centers onboarded yet.</td></tr>
              )}
            </tbody>
          </table>
        </div>
...
export function CreateAccessDialog({ role, onSaved }: { role: "center" | "staff"; onSaved: () => void }) {
  ...
  const submit = async (e: React.FormEvent) => {
    e.preventDefault();
    setBusy(true);
    const { error } = await supabase.from("allowed_managers" as any).upsert([{ email: email.toLowerCase().trim(), role }], { onConflict: "email" });
    setBusy(false);
    if (error) return toast.error(error.message);
    toast.success(`${email} can now sign up as ${role} — they'll be promoted automatically.`);
    setEmail("");
    onSaved();
  };
  ...
  return (
    <DialogContent className="bg-[#fbf6e7] border-4 border-foreground rounded-none max-w-md">
      <DialogHeader>
        <DialogTitle className="font-headline text-2xl uppercase">New {role} access</DialogTitle>
      </DialogHeader>
      <form onSubmit={submit} className="space-y-4">
        ...
        <p className="font-serif-news text-xs italic text-[#6b3e1a]">
          They create their own login at /signup using this email — the {role} role is granted automatically on first sign-in.
        </p>
        <DialogFooter>
